...
JavaScript utilise une architecture non-bloquante basée sur l'Event Loop.
Comment ça marche techniquement ?

Mise en pause locale : Quand le moteur rencontre await, il "suspend" l'exécution de la fonction async actuelle. Il sauvegarde son état (variables, curseur d'exécution).

Libération du thread : La fonction est retirée de la Call Stack (la pile d'exécution). Le thread principal (unique en JS) est désormais vide.

Gestion des autres tâches : Le moteur peut maintenant piocher dans la Task Queue (file d'attente) pour traiter :

- Des événements DOM (clics, scroll).
- Des timers (setTimeout).
- D'autres requêtes réseau.
- Le rendu graphique (UI).
  Reprise : Une fois que la Promise attendue est résolue, la suite de la fonction est placée dans la Microtask Queue pour être réinjectée dans la Call Stack dès que celle-ci est libre.
...
// Cette partie reste interactive même pendant les 5s du fetch :
document.addEventListener('click', () => console.log('Clic détecté !'));
```

L'interface reste fluide car le moteur traite l'événement click pendant que la requête réseau tourne en arrière-plan.  
...
Il faut voir le moteur (V8) non pas comme un bloc isolé, mais comme un chef d'orchestre dans un environnement (Navigateur ou Node.js).

### Voici les quatre composants qui font fonctionner l'asynchronisme :

#### 1. La Call Stack (La Pile)

C'est là que le code est exécuté. C'est une structure LIFO (Last In, First Out).

- Si vous appelez une fonction, elle est empilée.
- Quand elle finit, elle est dépilée.
- Problème : Elle est mono-threadée. Une seule tâche à la fois.

#### 2. Les Web APIs (Le Background)

Ce n'est pas du JavaScript pur, ce sont des outils fournis par le navigateur (ou l'OS via Node).

- fetch, setTimeout, les écouteurs d'événements (DOM).
  Quand vous faites un await fetch(), JS délègue la requête aux Web APIs et libère immédiatement la Call Stack.

#### 3. La Task Queue & Microtask Queue (La Salle d'attente)

Une fois qu'une Web API a terminé son travail (ex: la réponse réseau est arrivée), le callback (ou la suite du code après le await) est placé ici.

- Microtask Queue : Priorité haute (Promesses, await).
- Task Queue : Priorité basse (setTimeout, événements DOM).

#### 4. L'Event Loop (Le Surveillant)

Son rôle est unique et simple. Il vérifie en boucle :
Est-ce que la Call Stack est vide ?
Si oui, il prend la première tâche de la Microtask Queue et l'envoie dans la Call Stack pour l'exécuter.

### Pourquoi c'est vital de le savoir ?

Si vous écrivez une boucle de calcul intensive (ex: crypter un fichier) directement dans votre code, vous remplissez la Call Stack pendant 10 secondes. L'Event Loop ne pourra jamais envoyer les clics ou les rendus graphiques depuis la file d'attente vers la pile. L'interface gèle.

#### Résumé du flux async/await:

- await : "Je sors de la Call Stack et je laisse l'API gérer ça."
- API : Fait le travail en arrière-plan.
- Queue : "Le travail est fini, j'attends mon tour."
- Event Loop : "La pile est libre, reviens finir ton exécution."
...
- **Vite** is specialized for _development_.
...
- **The Benefit**: In your frontend code (main.ts), you can just fetch `/api/login`. You don't need to know _where_ the backend is running. Vite handles the handoff.
...
4.  **Automatic Sending:** For **every single future request** you make to that server (clicking a link, refreshing the page), your browser automatically attaches that cookie to the request headers.
    - _Browser:_ "Hey Server, give me my profile data. Also, here is my ticket: `session_id=xyz123`."
    - _Server:_ "Ah, ticket `xyz123` belongs to User 'Gentian'. Here is your data."
...
- **HTTP (Standard Web)**:
  ...
  - _Good for loading pages, images, and APIs._

- **WebSockets (Real-time)**:
  - Client connects once.
  - Server: "Here is a new message!" (immediately when it happens)
  - _Good for games, chat apps, and live dashboards._

### Why it matters for `ft_transcendence`

Since your project involves a **Pong game**, WebSockets are critical.

1.  **Game State**: When Player A moves their paddle, the server needs to tell Player B _immediately_. HTTP is too slow for this.
...
1.  **Remote Multiplayer Pong**:
    - If you want Player A on _Computer 1_ to play against Player B on _Computer 2_, you **must** use WebSockets.
    - HTTP requests are too slow to synchronize the ball and paddle positions 60 times a second.
    - _Note: If you only plan to support "Local Multiplayer" (two people sharing one keyboard), you might skip this for the game, but check your subject carefully._
...
- **Goal:** To catch bugs _automatically_ and _early_ before they are merged into the main codebase.
...
- **Goal:** Make _verifying_ the code fast and reproducible.
- **Includes:**
...
- **Goal:** Make _coding_ easy and comfortable.
- **Includes:**
...
- **The "Impact" Filter:** Developers usually skim PR titles or descriptions for changes that affect **their specific area**. If you are working on the Frontend and someone merges a major change to the Backend API, you _must_ read that PR to see if your code will break.
...
- **The Daily Stand-up:** Instead of reading every PR, teams use a 15-minute daily meeting to say: _"I merged the new Auth system yesterday; check the PR description if you need to see how the new login flow works."_
...
- Strictness: It prevents "phantom dependencies"—a situation where you can import a package that you didn't explicitly install because it was "hoisted" by npm. If it's not in your package.json, pnpm won't let you use it.
