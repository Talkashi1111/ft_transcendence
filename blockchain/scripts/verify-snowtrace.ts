...
async function flattenContract() {
  ...
  const { stdout } = await execAsync(`npx hardhat flatten contracts/TournamentScores.sol`);
...
async function checkVerificationStatus(guid: string) {
  ...
  if (result.status === '1') {
    ...
    console.log(
      `🔍 View on Snowtrace: https://testnet.snowtrace.io/address/${CONTRACT_ADDRESS}#code`
    );
  } else {
