...
describe('TournamentScores', async function () {
  ...
  it('Should get tournament matches', async function () {
    ...
    await tournamentScores.write.recordMatch([tournamentId, 1n, 'Alice', 2n, 'Bob', 10n, 5n], {
      account: owner.account,
    });
    await tournamentScores.write.recordMatch([tournamentId, 2n, 'Bob', 3n, 'Charlie', 7n, 8n], {
      account: owner.account,
    });
  ...
  it('Should reject invalid player IDs', async function () {
    ...
    await assert.rejects(
      async () => {
        await tournamentScores.write.recordMatch([1n, 0n, 'InvalidPlayer', 2n, 'Bob', 5n, 3n], {
          account: owner.account,
        });
      },
      /Invalid player1 ID/,
  ...
  it('Should reject when players are the same', async function () {
    ...
    await assert.rejects(
      async () => {
        await tournamentScores.write.recordMatch([1n, 1n, 'Alice', 1n, 'Alice', 5n, 3n], {
          account: owner.account,
        });
      },
  ...
  it('Should only allow owner to record matches', async function () {
    ...
    await assert.rejects(
      async () => {
        await tournamentScores.write.recordMatch([1n, 1n, 'Alice', 2n, 'Bob', 5n, 3n], {
    ...
    const hash = await tournamentScores.write.recordMatch([1n, 1n, 'Alice', 2n, 'Bob', 5n, 3n], {
      account: owner.account,
    });
    await publicClient.waitForTransactionReceipt({ hash });
  ...
  it('Should validate tournament ID', async function () {
    ...
    await assert.rejects(
      async () => {
        await tournamentScores.write.recordMatch([0n, 1n, 'Alice', 2n, 'Bob', 5n, 3n], {
          account: owner.account,
        });
      },
  ...
  it('Should validate alias length', async function () {
    ...
    await assert.rejects(
      async () => {
        await tournamentScores.write.recordMatch([1n, 1n, longAlias, 2n, 'Bob', 5n, 3n], {
          account: owner.account,
        });
      },
