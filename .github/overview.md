...
The backend exposes a REST‑style HTTP JSON API (Fastify routes such as POST /users/, GET /users/, etc.)
### REST
...
An API doesn't have to be REST — common alternatives include **GraphQL**, **gRPC**, **WebSockets**, **SSE**, **JSON‑RPC/RPC**, **message‑based queues**, and **SOAP**.

### Common alternatives
- **GraphQL** — single endpoint, client specifies exactly what fields it needs. Great for complex UIs and reducing round trips.  
- **gRPC** — binary, protobuf‑defined RPC calls (and streaming), high performance and strongly typed; ideal for inter‑service comms, less browser‑friendly.  
- **WebSockets** — full‑duplex persistent connection for real‑time two‑way communication (games, chat).  
- **Server‑Sent Events (SSE)** — simple server→client streaming (live feeds, notifications), one‑way.  
- **JSON‑RPC / XML‑RPC** — RPC style over HTTP where you call methods rather than CRUD resources. Simple and explicit.  
- **Message queues (AMQP/MQTT/Kafka)** — async, decoupled communication for background jobs and event-driven systems.  
- **SOAP / WS‑*** — XML, heavy enterprise features (transactions, WS‑Security); rarely used in modern web apps.

### When to choose which 🎯
- Use **REST** for simple, web‑native CRUD and broad compatibility.  
- Use **GraphQL** when clients need flexible queries and to avoid over/under‑fetching.  
...
### TLS termination
**TLS Termination** (Transport Layer Security Termination) is the process of decrypting encrypted traffic (HTTPS) at the entry point of your network—usually a load balancer or reverse proxy—before passing it on to your backend servers as plain text (HTTP).
...
---
## WebSockets
**WebSockets** is a communication protocol that provides a **full-duplex** communication channel over a single, long-lived connection.
...
- **Built application files** — compiled/transpiled output (`dist/` or `build/`).  
...
4.  **Reports:**
    ...
    *   If something fails, it gives you a red cross ❌ and sends you an email.
