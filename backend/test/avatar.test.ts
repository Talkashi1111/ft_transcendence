...
describe('Avatar Upload', () => {
  ...
  beforeAll(async () => {
    ...
    // Create a test user
    const uniqueEmail = `avatar-test-${Date.now()}@example.com`;
    const user = await prisma.user.create({
      data: {
        email: uniqueEmail,
        alias: `avatartest${Date.now()}`,
    ...
    // Generate auth token
    authToken = server.jwt.sign({ id: user.id, email: user.email });
  });
  ...
  describe('POST /api/users/me/avatar', () => {
    ...
    it('should accept valid JPEG file', async () => {
      ...
      expect(responseBody.avatarUrl).toBe(`/api/users/${testUserId}/avatar`);

      // Wait for rate limit to expire before next test
